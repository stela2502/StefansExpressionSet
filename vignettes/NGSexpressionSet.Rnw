...
information into the samples resp. annotation tables.
Please read the documentation on the R command line for all the different
options.

<<hclust_sample>>=
subs <- group.hclust ( subs, groups=4, name='hclust', type='gene' )

table(subs@annotation[,c('hclust 4 groups')])

subs <- group.hclust ( subs, groups=4, name='hclust', type='sample' )

table(
  apply(
	subs@samples[,c('GroupName', 'hclust 4 groups')],
	1, 	paste, 	collapse= "/gr.")
)
@
...
<<createRFgrouping_samples, cache=TRUE>>=
...
<<colour_example>>=
subs.C@usedObj[['colorRange']][['RFgrouping RFclust 1']]
@
...
<<colors_4>>=
subs.C <- colors_4(subs.C, 'RFgrouping RFclust 1', function(x){ bluered(x) } )
subs.C@usedObj[['colorRange']][['RFgrouping RFclust 1']]
@
...
plot.legend( subs, 'hist 4 groups', file="PlotSomething", pdf=T )
@
...
<<plot.legend>>=
## this will create a file 'x@outpath/PlotSomething_hist_4_groups.pdf'
plot.legend( subs, 'hclust 4 groups' )
@
\end{figure}
